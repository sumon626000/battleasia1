...
DECLARE
  v_id bigint;
  v_kill_rate_type public.match_kill_rate_type;
BEGIN
  IF NOT public.is_admin() THEN
    RAISE EXCEPTION 'Admins only';
  END IF;

  v_kill_rate_type := CASE p_payload->>'kill_rate_type'
    WHEN 'Manual' THEN 'Manual'::public.match_kill_rate_type
    WHEN 'Automatic' THEN 'Automatic'::public.match_kill_rate_type
    ELSE 'Automatic'::public.match_kill_rate_type
  END;
  ...
  IF p_match_id IS NULL THEN
    ...
    VALUES (
      ...
      COALESCE((p_payload->>'match_type')::public.match_type,'Free'::public.match_type),
      COALESCE((p_payload->>'game_mode')::public.match_game_mode,'Classic'::public.match_game_mode),
      COALESCE((p_payload->>'player_mode')::public.match_player_mode,'Squad'::public.match_player_mode),
      COALESCE((p_payload->>'reward_type')::public.match_reward_type,'RankBased'::public.match_reward_type),
      v_kill_rate_type,
      ...
      COALESCE((p_payload->>'status')::public.match_status,'Upcoming'::public.match_status),
      ...
      COALESCE((p_payload->>'platform_fee_pct')::numeric, 5),
      auth.uid()
  ...
  ELSE
    UPDATE public.matches SET
      ...
      match_type = COALESCE((p_payload->>'match_type')::public.match_type, match_type),
      game_mode = COALESCE((p_payload->>'game_mode')::public.match_game_mode, game_mode),
      player_mode = COALESCE((p_payload->>'player_mode')::public.match_player_mode, player_mode),
      reward_type = COALESCE((p_payload->>'reward_type')::public.match_reward_type, reward_type),
      kill_rate_type = v_kill_rate_type,
      ...
      status = COALESCE((p_payload->>'status')::public.match_status, status),
...
GRANT EXECUTE ON FUNCTION public.admin_save_match(bigint, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_save_match(bigint, jsonb) TO service_role;