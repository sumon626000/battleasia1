...
-- Add granular prize breakdown columns
ALTER TABLE public.match_participants
  ADD COLUMN IF NOT EXISTS kill_prize_bac bigint NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS win_prize_bac bigint NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS bonus_bac bigint NOT NULL DEFAULT 0;

-- Updated publish RPC: BattleAsia rule (kill_win auto + win_prize manual + bonus manual)
CREATE OR REPLACE FUNCTION public.admin_publish_match_result(
  p_match_id bigint,
  p_results jsonb,
  p_result_description text DEFAULT NULL,
  p_result_image_url text DEFAULT NULL
) RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_prize_pool numeric; v_total_income numeric;
...
BEGIN
  ...
  LOOP
    ...
    -- Kill Win (auto) = kills × per_kill
    v_kill_prize := v_kills * v_per_kill;
    -- Win Prize + Bonus only for Winner status
  ...
  UPDATE public.matches SET result_applied = true, status = 'completed', updated_at = now()
    WHERE id = p_match_id;
