...
BEGIN
  ...
  UPDATE public.profiles
    ...
    WHERE id = p_user_id;
  INSERT INTO public.admin_action_logs(admin_id, action, module, target_type, target_id, new_value)
  VALUES (auth.uid(), 'user_suspend', 'users', 'user', p_user_id::text,
...
BEGIN
  ...
  VALUES (auth.uid(), 'user_unsuspend', 'users', 'user', p_user_id::text);
END;$$;
...
BEGIN
  ...
  INSERT INTO public.admin_action_logs(admin_id, action, module, target_type, target_id, new_value)
  VALUES (auth.uid(), 'balance_adjust', 'wallet', 'user', p_user_id::text,
