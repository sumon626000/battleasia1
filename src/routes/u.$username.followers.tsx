...
function FollowList({ mode }: { mode: "followers" | "following" }) {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: rels } = await supabase.from("user_follows").select(otherCol).eq(col, prof.id);
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 space-y-4">
      <div className="hud-panel p-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display uppercase tracking-widest text-lg text-gold">
            {q.data.profile.username}'s {mode}
          </h1>
      ...
      <div className="hud-panel divide-y divide-border">
        {q.data.users.length === 0 ? (
          <div className="p-12 text-center text-foreground/50 text-sm">No {mode} yet.</div>
        ) : (
